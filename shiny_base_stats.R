...
# setwd("Z:/GitHub/Arcade_Basic_Stats")
# source("R/utils.R")
# source("data/ff_scoringhistory.R")
# ffscrapr_mapping <- read.csv2("data/ffscrapr_mapping.csv")
# Lade die E-Mail-Konfiguration aus der JSON-Datei
# email_config <- fromJSON("email_config.json")
# date <- "2021-06-14"
...
years <-2024
weeks <- 1:17
# load_base_stats_act(years)

### Past Data #####
# nflfastR::update_db(
#   dbdir = getOption("nflfastR.dbdirectory", default = "Z:/GitHub/nflfastR/"),
#   dbname = "pbp_db",
#   tblname = "nflfastR_pbp",
#   force_rebuild = TRUE,
#   db_connection = NULL
# )
# pbp <- tbl(dbConnect(SQLite(), "Z:/GitHub/nflfastR/pbp_db"), "nflfastR_pbp") |> filter(season %in% years) |> collect()
# pbp <- nflfastR::load_pbp(years)
# weekly <- nflfastR::calculate_player_stats(pbp, weekly = TRUE)
# weekly_def <- nflfastR::calculate_player_stats_def(pbp, weekly = TRUE)

# saveRDS(weekly, "Z:/GitHub/Arcade_Basic_Stats/data/weekly_99-22.rds")
# saveRDS(weekly_def, "Z:/GitHub/Arcade_Basic_Stats/data/weekly_def_99-22.rds")

### Expected Fantasy Points
# ep <- ffopportunity::ep_load(2006:max(years), type = "weekly")
# saveRDS(ep,"Z:/GitHub/Arcade_Basic_Stats/data/ep_99-22.rds")

#####

weekly_join <- readRDS("data/weekly_stats.rds")
weekly_join_war <- readRDS("data/weekly_stats.rds") |> select("player_id","player_name","recent_team","position","season","week","fantasy_points_ppr")
weekly_join_league_stats <- readRDS("data/weekly_stats.rds") |>  select("player_id","sleeper_id","player_name","recent_team","position","season","week","fantasy_points_ppr","attempts","completions","sacks","carries","receptions",
...
weekly_join_def <- readRDS("data/weekly_stats_def.rds")
weekly_join_def_war <- readRDS("data/weekly_stats_def.rds") |>  select("player_id","player_name","recent_team","position","season","week","fantasy_points_ppr")
...
# # Eine Funktion, um das Feedback per E-Mail zu senden
# send_feedback_email <- function(subject, message, contact_info) {
#   # Sende E-Mail mit den Einstellungen aus der Konfigurationsdatei
#   send.mail(from = email_config$smtpUser,
#             to = "info@arcadefantasy.de",
#             subject = subject,
#             body = paste("Nachricht: ", message, "\n\nKontaktinformationen: ", contact_info),
#             smtp = list(host.name = email_config$smtpServer,
#                         port = email_config$smtpPort,
#                         user.name = email_config$smtpUser,
#                         passwd = email_config$smtpPw))
# }
#####
  ...
  useToastr(),
    ...
    navbarPage(
      ...
      tabPanel("nflfastR Stats Defense",
               ...
               fluidRow(
                 column(3,pickerInput("team_tbl_nflfastR_def",
                                    label = "Choose a Team",
                                    choices = levels(weekly_join_def$recent_team),
                                    selected = levels(weekly_join_def$recent_team),
                                    options = list(`actions-box` = TRUE),
                                    multiple = TRUE)
               ),
      ...
      tabPanel("Player Stats Offense",
               ...
               fluidRow(column(3,
                               pickerInput("stat_tbl_off",
                                           label = "Choose a Stat",
                                           choices = levels(weekly_join$stat),
                                           selected = "Rushing/Receiving",
                                           options = list(`actions-box` = TRUE),
                                           multiple = T)
                               ),
                        ...
                        column(3,pickerInput("week_tbl_off",
                                             label = "Choose a Week",
                                             choices = levels(weekly_join$week),
                                             selected = 1:max(as.numeric(weekly_join$week)),
                                             options = list(`actions-box` = TRUE),
                                             multiple = T)
                        ),
               ...
               fluidRow(
                 ...
                 column(2,
                        pickerInput("war_season",
                                    label = "Choose a Season",
                                    choices = levels(weekly_join$season),
                                    selected = levels(as.factor(ifelse(lubridate::year(lubridate::today()) %in% levels(weekly_join$season),lubridate::year(lubridate::today()),max(levels(weekly_join$season))))),
                                    # options = list(`actions-box` = TRUE),
                                    multiple = FALSE)
                 ),
                 column(2,
      ...
      tabPanel("Values",
               ...
               # fluidRow(
               #   column(2,
               #          actionButton("values_x",
               #                       label = "Calculate"
               #          )
               #   ),
               #   column(2,
               #          downloadButton('values_y',
               #                         "Download")
               #   )
               # ),
      ...
      #####
      # Neuer Reiter für das Feedback #####
      # tabPanel("Feedback",
      #          fluidRow(
      #            column(12,
      #                   h3("Feedback und Suggestions"),
      #                   textInput("feedback_subject", label = "Betreff:", value = "Feedback / Suggestions für Arcade Fantasy Stats Lab", width = "100%"),
      #                   textAreaInput("feedback_message", label = "Nachricht:", value = "", placeholder = "Hier Feedback oder Suggestions eingeben...", width = "100%"),
      #                   textInput("contact_info", label = "Kontaktinformationen (optional):", value = "", placeholder = "Name, E-Mail, etc.", width = "100%"),
      #                   br(),
      #                   actionButton("send_feedback", "Feedback senden")
      #            )
      #          )
      # )
      #####
...
server <- function(input, output, session) {
  ...
  nflfastR_data_def <- reactive(
    weekly_join_def %>%
    # nflfastR::calculate_player_stats_def(nflfastR::load_pbp(as.numeric(input$season_tbl_nflfastR_def)), weekly=TRUE) %>%
      # left_join(nflfastR::fast_scraper_roster(as.numeric(input$season_tbl_nflfastR_def)) %>% select(player_id=gsis_id,season,pff_id,sleeper_id,espn_id)) %>%
      #   left_join(ffscrapr::espn_players() %>% mutate(espn_id=as.character(player_id)) %>% select(espn_id,pos)) %>%
      #   mutate(position = as.factor(as.character(pos)),
  ...
  observeEvent(input$war_calc, {
    ...
    war_df_pre <- tidyr::crossing(f_years = input$war_season, f_week = input$war_week) |>
      ...
      #   Ave_Week_Points = mean(Ave_Week_Points,na.rm=TRUE),
      #   Avr_Win_Percent = mean(Avr_Win_Percent,na.rm=TRUE),
      #   consistency_temp= 1-sd(WAR,na.rm=TRUE),
      #   WAR             = sum(WAR,na.rm=TRUE),
    ...
    # Update the global reactive value with the war_df dataframe
  ...
  output$WAR <- DT::renderDataTable({
    ...
    filter = "top"
        ...
        # formatRound(columns = c(16:18,21:25), digits = 2)%>%
  ...
  output$values<- DT::renderDataTable({
    ...
    if (!is.null(war_df)) {
      ...
      values_team <- bind_rows(
        ...
        mutate(
          OVR = rowSums(across(where(is.numeric)))
        ) %>%
        relocate(OVR, .before = QB) %>%
        arrange(-OVR)
    ...
    DT::datatable(values_team,
      ...
      DT::formatStyle(names(values_team),
                      background = if (isTRUE(input$light_mode)) styleColorBar(range(values_team[,2:length(values_team)]), '#FFC010',angle = -90) else styleColorBar(range(values_team[,2:length(values_team)]), '#915191',angle = -90),
      ...
      return(
      # Show a warning message in the center of the screen using shinytoastr
      shinytoastr::toastr_warning("Keine Daten verfügbar. Bitte zuerst Wins Above Replacement kalkulieren.", title = "ACHTUNG!", closeButton = TRUE,
  ...
  #   # Nachricht und Betreff aus den Eingabefeldern holen
  #   subject <- input$feedback_subject
  #   message <- input$feedback_message
  #   contact_info <- input$contact_info
  #
  #   # Falls die Nachricht leer ist, zeige eine Fehlermeldung
  #   if (message == "") {
  #     shinyjs::showNotification("Bitte gib eine Nachricht ein.", type = "error")
  #     return()
  #   }
  #
  #   # Feedback per E-Mail senden
  #   send_feedback_email(subject, message, contact_info)
  #
  #   # Zeige eine Erfolgsmeldung
  #   shinyjs::showNotification("Vielen Dank für dein Feedback!", type = "success")
  #
  #   # Zurücksetzen der Eingabefelder
  #   updateTextInput(session, "feedback_subject", value = "Feedback / Suggestions für Arcade Fantasy Stats Lab")
  #   updateTextAreaInput(session, "feedback_message", value = "")
  #   updateTextInput(session, "contact_info", value = "")
  # })
  #####
}
